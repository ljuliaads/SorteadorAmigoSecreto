...
function adicionarAmigo() { 
    ...
    if (nomes === '') { 
        alert('Digite pelo menos um nome válido!'); return;
     } 
     if (/\d/.test(nomes)) { 
        ...
        let listaNomes = nomes.split(',') .map(nome => nome.trim().replace(/\s+/g, ' ').toLowerCase()) .filter(nome => nome !== '');
        ...
        if (setNomes.size !== listaNomes.length) { 
         ...
         let nomesDuplicados = listaNomes.filter(nome => amigos.some(amigo => amigo.toLowerCase() === nome) );
        ...
        listaNomes.forEach(nome => { if (!amigos.map(a => a.toLowerCase()).includes(nome)) { amigos.push(nome.charAt(0).toUpperCase() + nome.slice(1));
         } });
     ...
     function atualizarLista() {
         ...
         lista.innerHTML = ''; amigos.forEach((nome) => { const li = document.createElement('li'); 
            li.textContent = nome; lista.appendChild(li); });
         } 
         
     function estourarConfete() {
         setTimeout(() => { confetti({ particleCount: 200, spread: 90, origin: { y: 0.6 }, colors: ['#ff0f0f', '#ff9800', '#ffeb3b', '#4caf50', '#2196f3', '#9c27b0'] }); }, 100); 
        } 
    ...
    let sorteado; 
     do { sorteado = amigos[Math.floor(Math.random() * amigos.length)];
    } 
